...
[^1]: 苏格兰两个多胜选举中的悖论奇异性
    ...
    [http://arxiv.org/abs/2305.20078](http://arxiv.org/abs/2305.20078)

    本文研究了苏格兰的两个多选举并找到了单调性悖论，无人出席和委员会大小悖论等异常现象。

    

    以排名投票方式为基础的选举中，单调性悖论等异常现象经过创建假设性示例并在多种选民行为模型下生成选举已得到广泛研究。但是，在真实世界中很少发现并分析到这样的投票悖论。本文研究了苏格兰的两个单记名选票选举，这些选举展示了单调性, 无人出席和委员会大小的悖论。这些悖论在真实世界选举中很少被观察到，这篇文章是关于多胜选举中此类悖论的第一个案例研究。
...
[^2]: VaR预测中条件分位数动态模型的建立与评估

    Modeling and evaluating conditional quantile dynamics in VaR forecasts. (arXiv:2305.20067v1 [q-fin.RM])

    [http://arxiv.org/abs/2305.20067](http://arxiv.org/abs/2305.20067)

    本文研究了VaR预测中条件分位数动态模型的建立与评估，通过仿真评估对称平均绝对偏差作为损失函数，以排名预测表现的优劣。实证应用中，采用移动预测窗口，在保持预测的分位数不变的情况下，以显着提高条件分位数的预测能力。

    

    我们专注于对于给定置信水平 τ，VaR的时变建模，评价标准是由条件均值和标准差标准化的收益率分布的分位数是否表现出可预测的动态特征。模型通过仿真进行评估，通过评估对称平均绝对偏差作为损失函数，以排名预测表现的优劣。在 Fama-French 25 等权重组合的实证应用中，采用移动预测窗口，在保持预测的分位数不变的情况下，除非违规的经验频率落在一个数据驱动的τ周围的区间之外，以显着提高条件分位数的预测能力。
...
[^8]: 基于贝叶斯MS-VAR过程的彩虹期权
    ...
    [http://arxiv.org/abs/2112.10447](http://arxiv.org/abs/2112.10447)

    本文提出了一种基于贝叶斯MS-VAR模型的彩虹期权和回顾期权的定价和对冲方法，该模型简单且依赖于经济变量。

    

    本文提出了基于贝叶斯马尔可夫状态转换向量自回归（MS-VAR） 过程的彩虹期权和回顾期权的定价和对冲方法。我们假设一个状态转换过程由同质马尔科夫过程产生。我们的模型的优点是依赖于经济变量且相对于以前的论文更为简单。
...
[^10]: 带溢出传染的违约系统
    ...
    [http://arxiv.org/abs/1709.09255](http://arxiv.org/abs/1709.09255)

    这个论文扩展了传统的传染模型，允许一个违约系统对其环境产生传染性影响，该模型的创新在于债务人的违约对更全球经济具有影响。

    

    在传统的传染模型中，违约系统在观测其随机环境条件下是马尔可夫过程，具有相互作用的强度。这要求环境自主演化，并且不受历史违约事件的影响。我们扩展了传统文献，并允许违约系统对其环境产生传染性影响。在我们的框架中，传染性可以被包含在违约系统内（即，从一对手到另一对手的直接传染），也可以从违约系统溢出到其环境中（间接传染）。每当需要在一个模型中捕捉某一类债务人的违约对更全球经济及其反向可能产生影响时，这种模型就具有兴趣。
