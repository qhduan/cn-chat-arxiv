...
[^4]: 健壮对冲生成对抗网络
    ...
    Robust Hedging GANs. (arXiv:2307.02310v1 [q-fin.CP])

    [http://arxiv.org/abs/2307.02310](http://arxiv.org/abs/2307.02310)

    本文介绍了一种健壮对冲生成对抗网络的扩展方法，通过对抗性方法处理数据生成过程中的不确定性。

    

    深度对冲的可用性为在各种实际市场条件下解决对冲问题开辟了新的前景。同时，无论是传统的随机模型还是市场生成器，都只是对市场现实的近似，容易出现模型规范错误和估计误差。这就引发了一个问题，如何通过自动化方式为建模设置提供可以应对预期分布与市场现实之间差异风险的工具。自动化的健壮化目前在许多投资问题上引起了越来越多的关注，但由于对风险管理的直接影响，这是一项复杂的任务。因此，毫无疑问，在这个问题上可以预见到更多的活动，以收敛于最佳实践的共识。本文提出了原始深度对冲框架的自然扩展，通过受对抗性方法的启示来处理数据生成过程中的不确定性。
...
[^5]: 功能时序噪音减少的方法
    ...
    [http://arxiv.org/abs/2307.02154](http://arxiv.org/abs/2307.02154)

    该论文提出了一种新的方法来在曲线时间序列中减少噪音，该方法基于功能主成分分析的框架。通过分离序列相关动态部分和噪音，该方法给出了底层去噪曲线的估计，并且在实验中表现出了优越性能。

    

    提出了一种在带有误差污染的曲线时间序列中噪音减少的新方法，该方法基于扩展功能主成分分析（FPCA）框架。我们利用功能时间序列的有限维动力学将观测曲线的序列相关动态部分与噪音分离开来。通过识别信号和特殊组分的子空间，我们构造了观察曲线时间序列在噪声子空间中的投影，从而得到了底层去噪曲线的估计。该投影在最小化均方积分误差方面是最优的。通过将我们的方法应用于模拟和实际数据，我们证明了去噪估计器的一致性并且胜过了现有的去噪技术。此外，我们还展示了它可以作为预处理步骤来提高预测的效果。
...
[^7]: 通过强化学习进行场外市场做市
    ...
    [http://arxiv.org/abs/2307.01816](http://arxiv.org/abs/2307.01816)

    本文提出了一个创新的强化学习方法来解决场外市场做市问题，并通过数值分析展示了在不同时间和库存水平下收益分布和买卖价差的最优策略。

    

    场外市场具有一种独特的特点，允许市场做市商根据订单大小调整买卖价差。然而，这种灵活性引入了复杂性，将市场做市问题转化为高维度的随机控制问题，提出了重大挑战。为了解决这个问题，本文提出了一种创新的解决方案，利用强化学习技术来应对场外市场做市问题。假设市场订单到达强度与买卖价差之间存在线性逆关系，我们证明了买卖价差的最优策略遵循高斯分布。我们应用两种强化学习算法进行数值分析，揭示了在不同的时间和库存水平下得到的收益分布和买卖价差。
...
[^14]: 关于Guyon-Lekeufack波动性模型的研究
    ...
    [http://arxiv.org/abs/2307.01319](http://arxiv.org/abs/2307.01319)

    本文研究了Guyon-Lekeufack波动性模型，证明了该模型在现实参数下有唯一的强解，并研究了所得波动性过程的正性。

    

    Guyon和Lekeufack最近提出了一种路径相关的波动性模型，并记录了它在拟合市场数据和捕捉特定事实方面的出色表现。瞬时波动性被建模为两个过程的线性组合，其中一个是加权过去价格收益的积分，另一个是加权过去平方波动的平方根的积分。每个加权使用两个指数核进行构建，反映长期和短期记忆。从数学上讲，该模型是一个由四个随机微分方程耦合的系统。我们的主要结果是该系统的唯一强（非爆炸性）解：模型对于现实参数值具有唯一的强解。我们还研究了所得波动性过程的正性。
...
[^17]: 密集乳化液中分形景观动力学的实验观察
    ...
    [http://arxiv.org/abs/2210.13667](http://arxiv.org/abs/2210.13667)

    本研究通过分析密集乳化液中的油滴轨迹数据，展示了高维分形景观的几何特征可以以无模型的方式进行可靠量化，并揭示这种几何结构驱动了油滴的复杂随机运动。

    

    许多软性和生物材料显示出所谓的“软玻璃”动力学；它们的成分经历异常的随机运动和复杂的合作重排。最近一个软玻璃材料的模拟模型，即一个成核泡沫，暗示它的泡沫的随机运动是由系统配置在高维空间中移动形成的分形能量景观。在这里我们展示了这样高维分形景观的显著几何特征可以通过多自由度的经验轨迹数据以无模型的方式进行探索和可靠量化。对于类似蛋黄酱的密集乳化液，通过分析获得的油滴轨迹定量地再现了使用计算模型生成的配置路径和其关联的能量极值的高维分形几何。这种几何结构反过来驱动了在实空间中观察到的油滴的复杂随机运动。我们的结果表明实验观测到的消解过程可以通过分形景观的几何特征来解释并可靠地量化。
