...
[^1]: 潘多拉盒子问题的最新进展：变体和应用

    Recent Developments in Pandora's Box Problem: Variants and Applications. (arXiv:2308.12242v1 [cs.GT])

    [http://arxiv.org/abs/2308.12242](http://arxiv.org/abs/2308.12242)

    潘多拉盒子问题是一个顺序搜索框架，最新研究主要关注其扩展和在市场设计、决策理论和机器学习等领域的应用。

    

    1979年，Weitzman引入了潘多拉盒子问题作为一个具有昂贵检查的顺序搜索框架。最近，潘多拉盒子问题特别受到经济学和计算领域交叉研究人员的关注。本综述文章概述了关于潘多拉盒子问题的最新文献，包括最新的扩展和在市场设计、决策理论和机器学习等领域的应用。
...
[^9]: 随机实验中治疗效果的半参数估计
    ...
    [http://arxiv.org/abs/2109.02603](http://arxiv.org/abs/2109.02603)

    本论文提出了一种用于估计随机实验中治疗效果的新的半参数方法，特别适用于结果分布厚尾、治疗效果小、样本量大且分配完全随机的情况。使用参数化模型估计治疗效果，导致了结果分布的半参数模型。我们推导了该设置下治疗效果的半参数效率边界，并提出了高效的估计器。在常量分位数治疗效果的情况下，其中一种高效估计器可以看作是分位数治疗效果的加权平均，权重与潜在结果密度函数的对数的二阶导数成反比。此外，我们还建议将Huber模型和修剪平均数扩展到包括不对称性。

    

    我们开发了一种新的半参数估计方法来估计治疗效果。我们关注的是结果分布可能是厚尾的、治疗效果可能很小、样本量很大且分配完全随机的情况。这种设置在最近的在线实验中特别引人关注。我们提出使用治疗效果的参数化模型，从而导致结果分布的半参数模型。我们推导了该设置下治疗效果的半参数效率边界，并提出了高效的估计方法。在常量分位数治疗效果的主导情况下，其中一种提出的高效估计器具有有趣的解释，可以看作是分位数治疗效果的加权平均，权重与潜在结果密度函数的对数的二阶导数成反比。我们的分析还建议将Huber模型和修剪平均数扩展到包括不对称性。
...
[^10]: 使用广义Bierens最大统计量对由条件矩限制所确定的参数进行推断
    ...
    [http://arxiv.org/abs/2008.11140](http://arxiv.org/abs/2008.11140)

    该论文提出了一种新的参数推断方法，通过结合模型选择和引导推断，对由具有高维条件仪器的条件矩限制所确定的参数进行推断。该方法通过解决一个数据相关的最大最小问题来优化渐近功率，并在两个实证例子中展示了其功效。

    

    许多经济面板和动态模型，如理性行为和欧拉方程，暗示了感兴趣的参数通过具有高维条件仪器的条件矩限制来确定。我们提出了一种新的参数推断方法，其中条件仪器的维度很高，并且没有关于哪些条件仪器是弱或无关的先验信息。基于Bierens（1990），我们提出了惩罚的最大统计量，并将引导推断与模型选择相结合。我们的方法通过解决一个数据相关的最大最小问题来为调节参数选择优化与感兴趣的n^{-1/2}本地备择方案的渐近功率。我们通过两个实证示例展示了我们方法的功效：跨期替代弹性和关于能力状况的理性无偏报告。基于第一个emp的广泛Monte Carlo实验
...
[^11]: 通过多变量循环奇异谱分析理解波动
    ...
    [http://arxiv.org/abs/2007.07561](http://arxiv.org/abs/2007.07561)

    我们引入了多变量循环奇异谱分析（M-CiSSA），提供了一种综合分析波动的框架。该方法是非参数的，可以识别共同的变化源，适用于相位错开、高度非线性和调制的序列。此技术在气候学、生物测量学、工程学等领域非常有用。

    

    我们引入了多变量循环奇异谱分析（M-CiSSA），提供了一个综合框架来分析波动，提取一组时间序列的基本成分，解开它们的变化来源，并评估它们在每个频率上的相对相位或周期位置。我们的新方法是非参数的，可以应用于相位错开、高度非线性和频率和幅度都有调制的序列。我们证明了一个唯一性定理，在共享信息的情况下，无需拟合因子模型，即可识别共同的变化源。这种技术在气候学、生物测量学、工程学或经济学等多个领域非常有用。我们通过一个合成示例和能源价格的实际数据分析展示了M-CiSSA的性能，以了解主要能源商品价格的驱动因素和共同变动。
