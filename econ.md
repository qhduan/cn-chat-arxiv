...
[^2]: 数学上的权力理论
    ...
    A mathematical theory of power. (arXiv:2401.16406v1 [econ.TH])

    [http://arxiv.org/abs/2401.16406](http://arxiv.org/abs/2401.16406)

    这篇论文提出了一种新的博弈论中的权力方法，利用利润改变机制来模拟合作和冲突，并通过衡量玩家合作的态度定义实际权力和潜在权力。

    

    本文提出了一种新的博弈论中的权力方法。通过一种称为F-game的利润改变机制来模拟合作和冲突。利用偏好的凸组合，可以衡量玩家合作的态度。然后我们定义实际权力和潜在权力作为系统不同状态之间的特殊关系。
...
[^4]: 图神经网络：估计网络异质性的理论及应用

    Graph Neural Networks: Theory for Estimation with Application on Network Heterogeneity. (arXiv:2401.16275v1 [econ.EM])

    [http://arxiv.org/abs/2401.16275](http://arxiv.org/abs/2401.16275)

    本文提出了一种新的图神经网络，用于模拟和估计网络异质性。它能够对网络中单位的决策或结果的变化进行建模，并考虑到邻居的条件。通过蒙特卡洛模拟，我们证明了这个方法的收敛速度，并应用于实证研究中的微金融计划参与。最终，我们提出了一种改进策略来选择初始信息接收者。

    

    本文提出了一种新颖的图神经网络应用于建模和估计网络的异质性。网络的异质性表现为单位的决策或结果的变化，不仅取决于其自身的属性，还取决于周围邻居的条件。我们描述了图神经网络估计器的收敛速度，并说明其在半参数因果推断中的适用性，包括处理异质治疗效应。通过蒙特卡洛模拟评估了我们估计器的有限样本性能。在与微金融计划参与相关的实证设置中，我们应用了新的估计器，以检验反事实政策的平均治疗效应和结果，并提出一种改进策略，用于在社交网络中选择初始信息接收者。
...
[^8]: 使用Copula测试线性回归模型中工具变量和回归器的外生性
    ...
    [http://arxiv.org/abs/2401.15253](http://arxiv.org/abs/2401.15253)

    本研究提出了一种基于Copula的方法来测试线性回归模型中的工具变量外生性。结果表明我们的方法在实际内生情况下能够高效地拒绝外生性假设，在外生情况下对外生性假设的拒绝率较低。对应用计量经济学家而言，这一方法具有重要价值。

    

    我们提出了一种基于Copula的方法来测试线性回归模型中的工具变量的外生性。我们展示了工具变量的外生性等价于其标准正态变换的外生性，并具有相同的累积分布函数值。然后，我们建立了一种用于测试工具变量外生性的Wald检验。我们使用模拟研究展示了我们的检验的性能。我们的模拟结果表明，如果工具变量实际上是内生的，我们的检验在5%的显著水平下大约93%的时间拒绝外生性假设。相反，当工具变量真正是外生的时候，它在数据有200个观测值时平均只有不到30%的时间拒绝外生性假设，在数据有1,000个观测值时几乎没有不到2%的时间。我们的结果证明了我们的检验的有效性，并对应用计量经济学家提供了重要价值。
...
[^9]: csranks: 一种用于估计和推断排名的R包
    ...
    [http://arxiv.org/abs/2401.15205](http://arxiv.org/abs/2401.15205)

    csranks是一个用于估计和推断排名的R包，可以用于构建置信集和进行排名回归的估计和推断。可以应用于量化排名不确定性和测量代际流动性等问题。

    

    本文介绍了用于估计和推断排名的R包csranks。首先，我们回顾了关于构建排名置信集的方法，包括边际和同时置信集以及关于tau最佳身份的置信集。其次，我们回顾了涉及排名回归的估计和推断方法。第三，我们描述了这些方法在csranks中的实现，并在两个例子中说明了它们的用途：一个是关于PISA国家排名不确定性的量化，另一个是使用排名回归测量代际流动性。
...
[^22]: 基于混合频率数据的宏观经济预测中的水库计算
    ...
    [http://arxiv.org/abs/2211.00363](http://arxiv.org/abs/2211.00363)

    本文提出了一种新的框架——基于水库计算的多频回声状态网络（MFESN），用于宏观经济预测中的混合频率数据，并证明MFESN模型在性能方面优于或可与现有方法相比，并提供了更好的美国GDP增长率的现货预测

    

    最近，宏观经济预测开始采用能够处理大规模数据集和不同发布时期系列的技术。MIDAS和DFM是两种允许建模具有非均匀频率的序列的主要现有方法。我们引入了一种新的框架，称为基于水库计算的多频回声状态网络（MFESN）。回声状态网络（ESN）是将递归神经网络公式化为具有随机状态系数的非线性状态空间系统的方法，其中只有观察映射受到估计。与MIDAS模型相反，MFESN要高效得多，允许纳入许多序列，而MIDAS模型容易受到维度诅咒。所有方法在针对美国GDP增长的广泛的多步预测练习中进行比较。我们发现，我们的MFESN模型在性能方面优于或可与其他方法相比，并提供了更好的美国GDP增长率的现货预测。
