...
| Ref | Title | Summary |
| --- | --- | --- |
| [^1] | [Monitoring multicountry macroeconomic risk.](http://arxiv.org/abs/2305.09563) | 该论文提出了一种多国量化分位数因子增强向量自回归（QFAVAR）模型，用于监测和评估全球冲击对各国宏观经济风险的影响。该模型的短期尾部预测较其他方法更准确。 |
| [^2] | [Executive Voiced Laughter and Social Approval: An Explorative Machine Learning Study.](http://arxiv.org/abs/2305.09485) | 本文探究了行政沟通中的发声笑声对于社会认可的积极影响，特别是当发生双向笑声时。结果表明，这种影响随着组织业绩的下降而增加。 |
...
[^1]: 监测多国宏观经济风险
    ...
    Monitoring multicountry macroeconomic risk. (arXiv:2305.09563v1 [econ.EM])

    [http://arxiv.org/abs/2305.09563](http://arxiv.org/abs/2305.09563)

    该论文提出了一种多国量化分位数因子增强向量自回归（QFAVAR）模型，用于监测和评估全球冲击对各国宏观经济风险的影响。该模型的短期尾部预测较其他方法更准确。

    

    我们提出了一种多国量化分位数因子增强向量自回归（QFAVAR）模型，用于建模宏观经济时间序列分布的跨国以及跨特征异质性。量化因子的存在允许以简明的方式总结这两种异质性。我们开发了两种后验推断算法，具有不同的估计精度和计算速度的平衡。使用欧元区的月度数据，我们建立了 QFAVAR 作为一种工具，用于评估全球冲击对国家层面宏观经济风险的影响，并证实了其良好的经验性质。尤其是，与具有对称高斯误差的 FAVAR 以及忽略宏观经济变量分位数之间共同变化的单变量量化自回归相比，QFAVAR 短期尾部预测更准确。我们还说明了由新型模型产生的量化脉冲响应函数和量化联系度量。
...
[^2]: 行政人员的发声笑声与社会认可：机器学习研究的探究。 (arXiv:2305.09485v1 [经济学.GN])
...
[^3]: 移动应用市场中免费增值策略的应用微观经济学论文研究

    Dissertation on Applied Microeconomics of Freemium Pricing Strategies in Mobile App Market. (arXiv:2305.09479v1 [econ.GN])

    [http://arxiv.org/abs/2305.09479](http://arxiv.org/abs/2305.09479)

    我的论文分析了移动应用的产品市场位置如何影响其定价策略，从而影响应用的货币化过程。我创建了一个新变量来衡量应用与同行之间的差异性，研究发现对成功的应用和不成功的应用的影响不同，并根据应用的累计安装量和公司知名度将应用分为市场领先和市场后继两组。

    

    在我的论文中，我将分析一个移动应用的产品市场位置如何影响其定价策略，从而影响应用的货币化过程。我利用自然语言处理和应用文本描述的k-mean聚类方法，创建了一个新的变量来衡量这些应用与同行之间的差异性。我创建了四个标准，即价格、累计安装量以及应用是否包含应用内广告和购买指示符。我发现，该效应对成功的应用和不成功的应用的影响略有不同。我用累计安装量和开发应用的公司来衡量成功。基于第三方排名和安装分布的形状，我设置了两个阈值，并将应用分为市场领先和市场后继应用。市场领先的样本由具有高累计安装量或由知名公司开发的应用组成，而市场后继的样本则由其它应用组成。
...
[^6]: Myerson正则下的Grenander型密度估计
    ...
    [http://arxiv.org/abs/2305.09052](http://arxiv.org/abs/2305.09052)

    本研究提出了一种 Myerson 正则下的 Grenander 型密度估计方法，该方法不需要调整参数，具有一致性和极小风险上限的优越性能。

    

    本研究提出了一种新的方法来估计私人价值从二价拍卖中的密度估计，与传统的基于平滑器的估计方法不同。我们引入了一个Grenander型估计器，构建在形状约束的基础上，这个形状约束是凸约束的形式。这个约束对应于拍卖理论中著名的Myerson正则条件，它等于拍卖物品销售的收入函数的凹性。我们的估计器是非参数的，不需要任何调整参数。在温和的假设下，我们建立了立方根一致性，并且证明了估计器渐近地遵循缩放的Chernoff分布。此外，我们还证明了估计器达到了极小化风险上限的收敛速度。
...
[^10]: 医学应用中回归不连续性设计的指南
    ...
    [http://arxiv.org/abs/2302.07413](http://arxiv.org/abs/2302.07413)

    本文提出了一份实践指南，介绍了在医疗背景下回归不连续性设计的分析方法，重点关注模糊的回归不连续设计和具有离散分数的回归不连续设计。

    

    我们提出了一份实践指南，旨在介绍医疗背景下回归不连续性设计的分析。我们首先介绍连续性框架和局部随机化框架中的关键概念、假设和估计方法。然后讨论这两个框架中的现代估计和推断方法，包括带宽或局部邻域选择方法、最优治疗效应点估计方法以及用于不确定性量化的鲁棒偏差校正推断方法。我们还概述了可用于支持关键假设的经验性假设检验。我们的讨论重点关注两个在生物医学研究中具有相关性的特征：（i）模糊的回归不连续设计，这经常出现在治疗方法基于临床指南但得分接近临界值的患者会根据其他规则接受治疗时；（ii）具有离散分数的回归不连续设计，在生物医学应用中普遍存在。
...
[^12]: GDP在G之前为什么会动？一切在于衡量
    ...
    [http://arxiv.org/abs/2212.06073](http://arxiv.org/abs/2212.06073)

    由于国防合同的延迟核算，导致GDP在G之前出现波动；VAR-identified shocks对库存的影响被错过，导致战斗多重效应估计较低。

    

    我们发现，国防新闻冲击对GDP的早期影响主要是由于商业库存的增加，承包商加速生产新的国防合同。这些合同直到交付付款（发生在2-3个季度之后）之前不会影响政府支出G。关于国防采购义务的新数据显示，合同授予会引起Granger因果VAR标识的G冲击，但不会引起国防新闻冲击。这意味着相对于G，早期GDP响应在很大程度上受到国防合同延迟核算的驱动，而VAR标识的G冲击错过了对库存的影响，从而得出较低的乘数估计。
...
[^15]: 数量的力量：对具有多个代理的公共物品拥有鲁棒性的机制
    ...
    [http://arxiv.org/abs/2101.02423](http://arxiv.org/abs/2101.02423)

    本文研究了在拥有$n$个独立代理的大型经济中提供公共物品的机制设计问题，提出了一种简单形式的机制，即调整均值阈值(AMT)机制，并证明了当供给成本增长速度小于$\sqrt{n}$时，该机制在经过一段时间后具有预算平衡和渐近有效性。

    

    本研究探讨在一个包含$n$个独立代理的大型经济中进行公共物品供给的机制设计问题。我们提出了一类主导策略激励兼顾后结果个体理性的机制，称为调整均值阈值（AMT）机制。我们表明，当供给成本增长速度小于$\sqrt{n}$时，AMT机制既在经历一段时间后的预算平衡，也是渐近有效的。相反地，当成本增长速度快于$\sqrt{n}$时，我们表明，任何满足激励兼顾、个体理性和经历一段时间后的预算平衡的机制必须具有收敛于零的供给概率，因此无法渐近有效。与其它机制（例如，次优机制）相比，AMT机制具有简单的形式，并且在信息方面更加鲁棒，这是因为AMT机制的构建仅依赖于价值分布的第一矩。
