...
| Ref | Title | Summary |
| --- | --- | --- |
| [^1] | [What would it cost to connect the unconnected? Estimating global universal broadband infrastructure investment.](http://arxiv.org/abs/2310.03694) | 本研究开发了一个全球模型，估计实现负担得起的普遍宽带所需的投资需求为约4180亿美元，其中大部分投资将用于新兴市场经济体和低收入发展中国家。 |
...
[^1]: 连接无连接者需要多少成本？估计全球普遍宽带基础设施投资

    What would it cost to connect the unconnected? Estimating global universal broadband infrastructure investment. (arXiv:2310.03694v1 [econ.GN])

    [http://arxiv.org/abs/2310.03694](http://arxiv.org/abs/2310.03694)

    本研究开发了一个全球模型，估计实现负担得起的普遍宽带所需的投资需求为约4180亿美元，其中大部分投资将用于新兴市场经济体和低收入发展中国家。

    

    大约有30亿公民仍然没有上网，相当于全球人口的40%左右。因此，提供互联网连接是可持续发展目标（目标9）的重要组成部分。本文开发了一个高分辨率的全球模型，以评估实现负担得起的普遍宽带所需的投资需求。结果表明，连接全球所有未连接的公民需要动员约4180亿美元（目标为每位用户每月40-50 GB，可靠性为95%）。额外投资的大部分用于新兴市场经济体（73%）和低收入发展中国家（24%）。据我们所知，本文首次提供了一个高分辨率的全球评估，以量化实现可持续发展目标9所需的普遍宽带投资。
...
[^3]: 基于稀缺心理的举措可以提高COVID-19疫苗接种

    Initiatives Based on the Psychology of Scarcity Can Increase Covid-19 Vaccinations. (arXiv:2310.03689v1 [econ.GN])

    [http://arxiv.org/abs/2310.03689](http://arxiv.org/abs/2310.03689)

    研究发现在意大利采取限量、限时发布COVID-19疫苗的举措能够推动疫苗接种活动，这利用了心理学和消费者营销的洞察力。

    

    背景：本研究调查了在2021年限量和限时发布COVID-19疫苗是否推动了意大利的疫苗接种活动。该策略利用了心理学和消费者营销的洞察力。方法：我们建立了一个覆盖意大利200天疫苗接种数据的原始数据集，包括“开放日”活动。开放日活动是指COVID-19疫苗在有限数量和特定时间发布的例子（通常是大型展馆或公共建筑）。我们的因变量是总接种和首次接种的数量与可接种人口的比例。我们的主要自变量是在特定地区特定日期举办开放日活动的存在。我们使用带有时间和地区固定效应的回归分析数据。分析结果对其他模型规范具有鲁棒性。
...
[^4]: 气候变化的一般均衡理论
    ...
    General Equilibrium Theory for Climate Change. (arXiv:2310.03650v1 [econ.TH])

    [http://arxiv.org/abs/2310.03650](http://arxiv.org/abs/2310.03650)

    本文提出了两个一般均衡模型，配额均衡和排放税均衡，并讨论了它们的性质和特点。通过设置适当的配额或税率，可以实现 Pareto 最优性。

    

    我们提出了两个一般均衡模型，配额均衡和排放税均衡。政府规定排放配额或税收，并停止进一步行动。配额均衡存在；分配排放产权对福利分配有强烈影响。如果唯一的外部性来自净排放总量，配额均衡是受到约束的 Pareto 最优的。每个配额均衡可以实现为排放税均衡，反之亦然。然而，对于某些税率，排放税均衡可能不存在，或者可能存在多个均衡解。通过设置适当的配额，通常可以实现配额均衡的完全 Pareto 最优性。
...
[^13]: 实现稳定的团结
    ...
    [http://arxiv.org/abs/2302.07618](http://arxiv.org/abs/2302.07618)

    团结成为实现稳定的充分条件。

    

    代理人可以形成联盟。每个联盟通过应用分配规则在其代理人之间分享其基金。分配规则假设代理人根据他们在相应分配问题中获得的分配对联盟进行排序，从而引出了联盟形成问题。我们将引起一类稳定联盟形成问题的分配规则特征化为满足一个公理的规则，该公理形式化了团结原则。因此，团结成为实现稳定的充分条件。
