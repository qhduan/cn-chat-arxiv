...
[^2]: 社会偏好与有意的随机行为
    ...
    Social Preferences and Deliberately Stochastic Behavior. (arXiv:2304.14977v1 [econ.TH])

    [http://arxiv.org/abs/2304.14977](http://arxiv.org/abs/2304.14977)

    本研究提出了一个随机选择模型，可以识别促进互惠行为的动机，并探索刻意随机化的替代动机，可以表示社会偏好，区分随机不平等厌恶行为和随机羞愧缓解行为。

    

    本研究提出了一个可行的随机选择模型，以确定促进互惠行为的动机，并探索超出先验公平考虑的刻意随机化的替代动机。为了表示社会偏好，我们采用加性扰动的效用模型，包括期望效用和非线性成本函数之和，其中效用函数完全自私，而成本函数则取决于社会偏好。使用成本函数，我们研究了随机选择模式，以区分随机不平等厌恶行为和随机羞愧缓解行为。此外，我们还讨论了我们的模型如何补充最近实验性证据的先后公平性考虑。
...
[^6]: 非传递偏好和随机合理性：一种行为等价性。
    ...
    [http://arxiv.org/abs/2304.14631](http://arxiv.org/abs/2304.14631)

    本文揭示出在单调性假设下，非传递偏好的行为等价于一个传递的随机选择模型。

    

    长期以来，非传递选择一直是经济学中一个引人注目的领域。然而，确定非传递选择是否代表个体的偏好是一项困难的任务，因为选择数据本质上是随机的。本文表明，在单调性假设下，非传递偏好的行为等同于一个传递的随机选择模型。特别地，非传递偏好通常被解释为偏好的强度，因此我们假设选择替代品是按照非传递偏好的比例选择的。这一结果的一个含义是，人们无法区分“注意力互补性”和“需求互补性”。
...
[^8]: 评估文本挖掘和技术分析对金融时间序列预测的影响
    ...
    [http://arxiv.org/abs/2304.14544](http://arxiv.org/abs/2304.14544)

    本文评估了文本挖掘和技术分析在预测金融时间序列中的准确性，比较了这两种方法预测S&P500未来价格的优缺点，并表明FinBERT模型表现较好，有潜力革新金融分析和预测。

    

    预测金融时间序列是金融和经济领域的重要研究方向，旨在预测金融市场的走势。本文研究了文本挖掘和技术分析在预测金融时间序列中的准确性，并聚焦于S&P500股票市场指数，该指数追踪美国最大的上市公司的表现。本研究比较了两种预测S&P500未来价格的方法：文本挖掘和技术分析。文本挖掘利用自然语言处理技术从财经新闻中提取有意义的信息，技术分析则使用历史价格和成交量数据进行预测。研究考察了两种方法的优缺点，并分析了它们在预测S&P500中的性能。FinBERT模型在S&P500价格预测方面表现较好，其根均方误差较低，有潜力利用财经新闻数据进一步革新金融分析和预测。
...
[^10]: 通过信息设计实现线性二次高斯博弈中的稳健社会福利最大化
    ...
    [http://arxiv.org/abs/2303.05489](http://arxiv.org/abs/2303.05489)

    该论文研究了一个设计者具有部分对代理人效用的知识的情况下，如何通过信息设计来实现线性二次高斯博弈中的稳健社会福利最大化。

    

    不完全信息博弈中的信息设计包括一个旨在通过从设计的概率分布中生成的信号来影响玩家行为的设计师，以使其目标函数被优化。我们考虑一个设计者具有部分对代理人效用的知识的情况。我们通过针对最坏情况支付来解决有关玩家偏好的不确定性，制定一个稳健的信息设计问题。如果玩家具有依赖于玩家动作和未知的与支付相关的状态的二次支付，以及在状态实现的条件下遵循高斯分布的状态信号，则在二次设计目标下的信息设计问题是一个半定规划（SDP）。具体而言，我们考虑线性二次高斯（LQG）游戏中的支付系数上的椭球体扰动。我们展示了这导致一个可处理的稳健SDP公式。进行了数值研究以确定关系
...
[^14]: 一种自然适应性的集体决策过程
    ...
    [http://arxiv.org/abs/2103.14351](http://arxiv.org/abs/2103.14351)

    提出了一种灵活的集体决策过程，类似于自然过程和机器学习中的算法，其产生的集体决策经验分布近似于极大抽签，且在时间推移中使用极大抽签作为集体决策的概率呈指数增长。

    

    考虑一个装满小球的瓮，每个小球上标有若干可能的集体决策。现在，让一位随机选民从罐子里抽出两个小球，并选择她更喜欢的作为集体决策。将输掉的小球重新标记为集体决策，在将两个小球放回罐子中并重复。偶尔，将随机抽出的小球重新标记为随机集体决策。我们证明了此过程产生的集体决策的经验分布近似于极大抽签，这是Peter C. Fishburn (Rev. Econ. Stud.，51（4），1984)提出的一种著名的概率性投票规则。事实上，在第$n$轮中采用极大抽签作为集体决策的概率呈指数增长。该过程比传统的投票规则更灵活，并具有类似于生物学、物理学和化学研究中的自然过程以及机器学习中提出的算法的强大相似性。
