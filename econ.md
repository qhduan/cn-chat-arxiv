...
[^5]: 正式协变量基准测试以限制忽略变量偏差
    ...
    [http://arxiv.org/abs/2306.10562](http://arxiv.org/abs/2306.10562)

    本文介绍了正式协变量基准测试的原理和细节，强调了解释残差部分时研究人员所面临的困难。这个方法能限制忽略变量偏差并给出一个未观察到的混杂因子的强度范围。

    

    协变量基准测试是关于忽略变量偏差的敏感性分析的重要部分，可以使用有关观察到的协变量的信息和判断来限制未观察到的混淆因素的强度。通常在将未观察到的混杂变量在观察到的协变量集上进行残差化后进行正式的协变量基准测试。本文解释了该过程的原理和细节，并强调了研究人员在推理未观察到的混淆因素的残差部分时所面临的解释困难。文章通过几个实证例子来阐述所有的观点。
...
[^17]: 结构方程模型方法研究Dhaka市用户对顺风车服务接受度的认知
    ...
    [http://arxiv.org/abs/2210.04086](http://arxiv.org/abs/2210.04086)

    本研究使用结构方程模型探究Dhaka市用户对顺风车服务的认知，结果表明用户对安全和保障的关注程度高于服务表现。

    

    本研究旨在建立一个多元统计模型，评估Dhaka市用户对顺风车服务接受度的认知。研究根据用户报告的态度和风险认知开发了结构化问卷。共收集了350个来自Dhaka市顺风车服务用户和利益相关者的正态分布回答。通过确定偏好问卷采访受访者，以表达他们对顺风车服务的体验和意见。采用结构方程模型（SEM）验证研究假设。使用统计参数和多种试验选择最佳SEM。应用相对重要性指数（RII）方法纠正SEM。 在SEM内，通过两个潜在和十八个观察变量来衡量顺风车服务的质量。潜在变量“安全与保障”对整体质量的影响大于“服务表现”。
...
[^22]: 通过策略无虚拟性表征顶级周期
    ...
    [http://arxiv.org/abs/2108.04622](http://arxiv.org/abs/2108.04622)

    本研究证明了，在温和条件下，顶级周期是唯一的不征用的策略性SCC，其结果仅取决于交量两两比较之间的量化。

    

    Gibbard和Satterthwaite已经证明，满足非征用（即函数的范围与其余域相一致）和策略无虚拟性（即选民通过歪曲其偏好永远不会更好）的唯一单值社会选择函数（SCF）是独裁主义。本文考虑根据Fishburn的偏好扩展进行策略证明的集合值社会选择对应（SCC），特别是顶级周期，一个返回弱多数关系的传递闭包的最大元素的有吸引力的SCC。我们的主要定理暗示，在温和条件下，顶级周期是唯一的不征用的策略性SCC，其结果仅取决于交量两两比较之间的量化。此结果有效地将Gibbard-Satterthwaite的不可能性转变为通过从SCF移动到SCC完全表征顶级周期。它作为更强的策略证明方法下传递SCC的更普遍的表征的推论而得出。
