...
[^16]: 测试公司行为
    ...
    [http://arxiv.org/abs/2301.06720](http://arxiv.org/abs/2301.06720)

    该论文通过模型选择和评估测试了公司的行为，提出了Rivers和Vuong（2002）（RV）模型在错误规范下的优势，并通过弱仪器性质和弱仪器诊断提供了对退化和仪器强度的理解。

    

    在评估不完全竞争市场的政策时，需要了解公司的行为。虽然研究人员通过模型选择和评估来测试行为，但我们提出了Rivers和Vuong（2002）（RV）模型在错误规范下的优势。然而，RV的退化使得推理无效。通过对测试的新定义，我们将退化与仪器强度联系起来，推导了RV的弱仪器性质，并通过将Stock和Yogo（2005）的框架拓展到模型选择中提供了弱仪器的诊断。我们使用常见的仪器组测试垂直行为（Villas-Boas，2007）。其中一些是弱的，提供不了力量。强仪器支持制造商设定零售价格。
...
[^17]: 重新审视事件研究设计：稳健且高效的估计方法
    ...
    [http://arxiv.org/abs/2108.12419](http://arxiv.org/abs/2108.12419)

    本研究对事件研究设计提出了稳健且高效的估计方法，该方法适用于时间变化的控制变量、三重差异设计和某些非二进制治疗。在应用研究中，研究美国税务回扣对消费反应时，发现名义消费倾向边际约为文献中估计值的一半。

    

    我们开发了一种差异法和错落收治采用的设计框架，考虑到异质性因果效应。我们发现，传统的基于回归的估计方法在没有强制要求治疗效应同质性时无法提供相关估计的无偏估计。然后，我们推导了一种解决这一挑战的有效估计方法，当治疗效应的异质性未受限制时，该方法具有直观的“插补”形式。我们表征了该估计器的渐近行为，提出了推断工具，并开展了用于识别假设的测试。我们的方法适用于有时间变化的控制变量、三重差异设计和某些非二进制治疗。通过模拟研究和一个应用研究，我们展示了我们结果的实际相关性。在研究美国税务回扣对消费反应时，我们发现，在第一季度，名义消费倾向边际在8%到11%之间，大约是文献中估计值的一半。
...
[^19]: 用于合成控制的$t$检验
    ...
    [http://arxiv.org/abs/1812.10820](http://arxiv.org/abs/1812.10820)

    我们提出了一个用于合成控制的$t$检验方法，它易于实施，具有鲁棒性，并且适用于规范错误的情况和平稳与非平稳数据。在模拟实验中表现出良好的小样本性能，并相对于其他方法而言效果良好。我们通过研究碳税对排放的影响展示了该方法的实用性。

    

    我们提出了一种实用且鲁棒的方法，用于对合成控制估计的平均处理效应进行推断。我们开发了一个$K$-折交叉拟合的偏差校正程序。为了避免长期方差的困难估计，推断基于自归一化$t$统计量，该统计量在渐近上服从关键的$t$分布。我们的$t$-检验易于实施，证明了在规范错误下的鲁棒性，并且适用于平稳和非平稳数据。在基于应用的模拟中，我们展示了这种方法在小样本情况下的优良性能，并且相对于替代方法表现良好。我们通过重新思考碳税对排放的影响来展示了$t$-检验的有用性。
