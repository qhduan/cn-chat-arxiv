...
[^4]: 两家垄断保险公司在强制性网络数据共享制度下对数据质量的激励效应
    ...
    [http://arxiv.org/abs/2308.00795](http://arxiv.org/abs/2308.00795)

    在强制数据共享的情况下，两家保险公司的数据质量投资可能不一样，对于强制数据共享的有效性存在担忧。

    

    我们研究了数据共享政策对网络保险市场的影响。这些政策旨在解决网络威胁数据的稀缺性问题，这对于管理网络风险至关重要。我们提出了一个柯诺垄断竞争模型，其中两家保险公司选择他们提供的保险数量（即生产水平），以及他们投资于确保索赔成本数据质量的资源（即他们生产成本的数据质量）。我们发现，强制数据共享有时会导致只有两家保险公司中的一家投资于数据质量，而当信息共享不是强制性的时候，两家保险公司都会进行投资。这引发了关于强制数据共享的可取之处的担忧。
